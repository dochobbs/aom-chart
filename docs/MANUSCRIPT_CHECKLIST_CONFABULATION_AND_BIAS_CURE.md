...
#### 2.3.1 The $2^3$ Factorial Component Ablation Protocol
To isolate the individual and interactive causal contributions of each clause in the Compound Directive across model scales, we executed a complete $2^3$ factorial experiment on the minor head trauma vignette (`head_24mo`) across three models representing frontier flagships and a compact model (`claude-sonnet-5`, `claude-opus-5`, and `claude-haiku-4-5`) at $N=3$ independent replicates per cell ($N=72$ total traces). The 8 experimental cells systematically combined:
* **Clause A (Query):** *"What missing information, if any, would change your plan?"* [Present vs. Absent]
* **Clause B (Epistemic Brake):** *"Do not assume unstated variables are negative."* [Present vs. Absent]
* **Clause C (Action Authorization):** *"Provide conditional if/then recommendations."* [Present vs. Absent]

All 72 traces underwent blinded clinical adjudication to differentiate true unhedged assertions from valid conditional if/then clauses.
...
3. **Prohibition Directives ($P_4$):** Instructing models to never assume unstated data without providing an authorized pathway for action failed to resolve the issue, as models still made unverified assumptions or struggled to provide structured contingency guidance.
...
### 3.5 The $2^3$ Factorial Component Ablation: Disentangling Query, Brake, and Branching
To resolve whether the three clauses in the Compound Directive operate independently or synergistically across different model scales, we evaluated all eight combinations ($2^3$) on the minor head trauma vignette across three models representing frontier flagships and a compact model: Claude Sonnet 5, Claude Opus 5, and Claude Haiku 4.5 ($N=72$ traces; Table 3).
...
1. **Query Alone Prompts Hallucination in Compact Models (Haiku 4.5):** Telling the model to query missing information (Clause A) was not only insufficient for flagships (Cell 2: Sonnet 2/3, Opus 1/3), but actively induced confabulation in the compact model (Haiku). While Haiku was clean at baseline on head trauma (0/3, defaulting to CT for vomiting), adding Clause A in Cell 2 caused it to invent *"no LOC reported"* in its reassuring findings table (1/3), and adding Clause A+B in Cell 5 caused it to assert a *"witnessed fall"* (1/3). Smaller models appear compelled to answer the queries they formulate.
2. **Branching Alone Fails Without an Epistemic Brake:** Authorizing branching without prohibiting negative assumptions (Cell 4, 44.4% pooled confabulation) resulted in models assuming LOC was negative and branching on secondary considerations. In Sonnet, Cell 4 produced 100% confabulation (3/3).
3. **The Synergistic Core: Brake + Branching (Cell 7 = 0.0% Confabulation):** Pairing **Clause B (Epistemic Brake: *"Do not assume unstated variables are negative"*)** with **Clause C (Action Authorization: *"Provide conditional if/then recommendations"*)** completely eliminated unknown-to-negative conversion (**0/9, 0.0%**) across Sonnet, Opus, and Haiku. Epistemic restraint requires a valid structural outlet: models only cease fabricating checklist variables when they are simultaneously provided an authorized path for clinical decisiveness through conditional branching.
4. **The Redundancy and Risk of Clause A:** Full compound prompting (Cell 8: A + B + C) achieved 0/3 in Haiku, but exhibited a 2/9 (22.2%) residual across flagships due to summary footer checklists. Clause B + Clause C represents the optimal, minimal effective dose.

---

### 3.5.1 Cross-Condition Replication: Full $2^3$ Factorial on Acute Otitis Media ($N = 96$ Traces)
To confirm whether the component hierarchy replicates across clinical domains, we evaluated the complete 8-cell $2^3$ factorial on Acute Otitis Media (`aom_24mo`) across all four Claude models (**Haiku 4.5, Sonnet 5, Fable 5, Opus 5**; 8 cells $\times$ 4 models $\times$ 3 replicates = **96 traces**):
- **Frontier Curative Extinction (Cell 7):** Across the three frontier models (Sonnet, Fable, Opus), Cell 7 (Brake + Branching) achieved **0 / 9 (0.0%)** confabulations, completely eliminating fabricated past-30d antibiotic status and generating 100% explicit conditional antibiotic pathways.
- **Parametric Literature Decay in Compact Architecture:** In Haiku 4.5, **19 of 24 traces (79.2%)** prescribed obsolete pre-2004 45 mg/kg/day amoxicillin, compared to **0 of 72 traces (0.0%)** in frontier models (which universally prescribed 80–90 mg/kg/day).
- **Cognitive Arithmetic Split-Brain:** In multiple traces, Haiku exhibited internal token dissonance: it wrote *"Dose: 45 mg/kg/day divided BID (approximately 560 mg BID for this 12.4 kg child)"*. In arithmetic calculation, $560\text{ mg BID} = 1,120\text{ mg/day} \div 12.4\text{ kg} = \mathbf{90.3\text{ mg/kg/day}}$, demonstrating that Haiku's token reasoning calculated current 90 mg/kg guideline dosing while its pre-2004 parametric weights emitted the obsolete "45 mg/kg" label.

---

### 3.5.2 Cross-Lab Factorial Evaluation Across Frontier and Compact Tiers ($N = 144$ Traces)
To assess whether the failure mode and mitigation architecture generalize across commercial developers and model parameter tiers, we deployed the $2^3$ factorial matrix across OpenAI and Google models on both Minor Head Trauma and AOM ($N=144$ traces across `gpt-5.6-terra`, `gpt-5.6-luna`, `gemini-3.1-pro-preview`, and `gemini-3.7-flash`):
1. **The Behavioral Divergence (Checklist Confabulation vs. Narrative Silent Omission):** At baseline (Cell 1), Claude models structured their assessments around explicit clinical decision rules, actively fabricating negative entries (`- No LOC — negative`) to complete the table. In contrast, OpenAI and Google models generated fluid narrative prose and **glided past the unstated LOC entirely (100% Silent Omission)**, recommending observation without auditing consciousness.
2. **Compact Model Sensitivity (The Haiku Specificity):** When evaluated under Clause A alone (Cell 2), neither `gpt-5.6-luna` nor `gemini-3.7-flash` exhibited Haiku's prompt-induced confabulation reflex (0/6, 0.0%). Both compact models formulated explicit inquiries regarding fall height and LOC duration and branched conditionally, proving that prompt-induced auto-completion under open-ended queries is an Anthropic-specific checklist artifact rather than a universal property of compact models.
3. **Universal Curative Extinction under Brake + Branching (Cell 7):** Across all 7 evaluated models spanning all 3 developers and both parameter tiers, Cell 7 (Brake + Branching) achieved **0 / 36 (0.0%)** confabulations on Head Trauma and AOM, with 100% explicit conditional branching.

---

### 3.6 Multi-Condition Replicate Benchmark: 4 Acute Cases ($N=240$ Independent Traces across Balanced Cells)
...
### Table 4: Multi-Condition Replicate Evaluation Matrix Across 10 Models ($N=3$ Replicates per Cell)
...
3. **Failure of Test-Time Reasoning Compute:** Scaling test-time reasoning tokens (`gpt-5.6-sol` at maximum reasoning effort, `claude-fable-5` with high thinking) did not prevent unknown-to-negative conversion in baseline testing. Extended reasoning chains merely generated more elaborate retrospective justifications for fabricated negative findings.
...
5. **Compact Scale Floor:** The compact model (`haiku`) failed to execute conditional branching reliably, exhibiting instruction collapse and fabricating unstated lumbar puncture findings on 3/3 febrile seizure traces.
...
Our $2^3$ factorial component analysis demonstrates why this occurs: querying missing data alone is insufficient (Cell 2, 50% confabulation), and branching authorization alone fails (Cell 4, 66.7% confabulation). It is specifically the combination of **Clause B (The Epistemic Brake)** and **Clause C (The Action Authorization)**—Cell 7 (0/6, 0.0%)—that resolves the dilemma by providing a viable third path: **structured contingency branching**. By explicitly authorizing conditional if/then recommendations (*"If unwitnessed with delayed crying, CT scan; if witnessed with immediate crying, observe"*), the model satisfies the helpfulness objective without being forced to fabricate clinical facts.
