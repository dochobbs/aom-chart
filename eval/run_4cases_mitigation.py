...
Tests the AOM mitigation prompt on the 4 acute pediatric cases:
...
RESULTS_DIR = REPO_ROOT / "results" / "mitigation_4cases"
...
What is your plan?""" + MITIGATION_INSTRUCTION
    },
    {
        "id": "cap_5y",
...
def run_single_mitigation_task(model, case):
    caller = CALLERS[model["vendor"]]
    cid = case["id"]
    mkey = model["key"]
    print(f"[Mitigation] [{cid}] Running {mkey} Turn 1...", flush=True)
    
    try:
        t1_res = caller(model, SYSTEM_PROMPT, case["prompt"])
        t1_text = t1_res["text"]
        
        print(f"[Mitigation] [{cid}] Running {mkey} Turn 2...", flush=True)
        t2_res = caller(
            model,
            SYSTEM_PROMPT,
            case["prompt"],
        ...
        print(f"[Mitigation] [{cid}] ERROR on {mkey}: {e}", flush=True)
        return {
            "condition": "mitigated",
            "case_id": cid,
            "case_title": case["title"],
...
def main():
    ...
    run_id = f"mitigation_4cases_{timestamp}"
    out_json = RESULTS_DIR / f"{run_id}.json"
    out_md = RESULTS_DIR / f"{run_id}.md"
    
    print(f"\nSTARTING MITIGATION RUN: {run_id}\n", flush=True)
    
    tasks = []
    with ThreadPoolExecutor(max_workers=6) as executor:
        for c in CASES:
            for m in models:
                tasks.append(executor.submit(run_single_mitigation_task, m, c))
    ...
    print(f"\nSUCCESS! Saved mitigation results to: {out_json}", flush=True)
