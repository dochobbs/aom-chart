...
To see whether this was just a quirk of raw research models, I took the exact same case and the same sentence to the dedicated clinical decision support tools physicians actually pay for: OpenEvidence, UpToDate Expert AI, AMBOSS, Doximity, Glass Health, and Vera Health. 

On the bare chart, OpenEvidence had made the exact same fabrication error as my frontier models: in two out of three runs, it asserted "no amoxicillin in the prior 30 days" as settled fact and mandated immediate treatment, even citing a *New England Journal* review for an antibiotic rule that doesn't exist. 

The moment I added the one-sentence constraint, OpenEvidence's fact fabrication fell to zero. It stopped asserting the history, explicitly named the 30-day antibiotic gap as the critical missing variable, and shifted from an immediate mandate to shared decision-making with the mother. The fix wasn't a model-tuning trick. It was a behavioral rule that works everywhere.

The same experiment mapped the fix's boundary, and the boundary teaches as much as the fix. The dead dose survived the new sentence. So did the age mistake. Prompting changed *behavior* (the habit of papering over gaps) because behavior lives in the instructions. It could not change *beliefs* (the stale dose, the broken threshold) because those live in the training data, where no prompt reaches.

That is where specialized clinical tools earned their keep. When we tested the dedicated medical engines, the stale 45 mg/kg dose vanished completely: 0 percent across more than a hundred clinical traces. Their medical retrieval layer cleanly superseded the 2004 text. But retrieval alone did not stop them from making up chart facts. Dosing requires medical knowledge, which retrieval fixes. Asking about missing facts requires an instruction, which only the prompt can provide. Medicine will recognize the working half. It's a checklist: a forcing function that makes an invisible step mandatory. Surgical safety checklists roughly halved mortality in the original multi-country study, not because surgeons lacked knowledge but because completeness under pressure skips steps unless something forces the pause. The difference is deployment speed. Getting checklists into operating rooms took a decade of culture war. I patched four AI models in an afternoon.
...
My own testing produced a parallel warning about the fragility of single observations. When evaluating OpenEvidence across three runs of the exact same chart, Run 1 mandated immediate treatment for 10 days, Run 2 flipped to 5 days of observation, and Run 3 flipped back to immediate treatment. A single-run trial would have rated the tool either perfectly concordant or totally non-compliant depending entirely on which run it sampled. Multi-replicate testing is the only way to catch stochastic drift in clinical decision tools.
...
You can now see this interface principle running in production. When I ran our chart through UpToDate Expert AI, it didn't wait for me to prompt it with a clever sentence. Its system opened by stating its clinical premises in bold: *"I am assuming the child has reliable caregivers and access to follow-up within 72 hours."* Below its plan, it generated interactive, clickable chips: `[follow-up within 72 hours is not reliable]`, `[child received a beta-lactam in the last 30 days]`. 

Clicking the chip flipped the recommendation in real time. UpToDate did not treat the interaction as an open-ended essay contest. It treated it as a structured clinical encounter where the software's job is to surface its assumptions and make the human confirm them.
