# Proposed edit of the anchor essay — Grok 4.6 (not the canonical draft)

Grok 4.6 pass against `share/compared_to_whom.md`, 2026-08-22. Diff is the review notes: Fable clause, mitigation denominators, Fable not in the rerun, Goh prompting caveat, first-person closer, plus a few magazine leftovers cut. Original restored in place. Do not post without his review.

No em-dashes outside verbatim model quotes. `blog_post.md` is the technical companion. Every AOM quote verbatim from the packet; every literature number Tier 1–2 per `docs/human_parallels.md` audit (Berdahl is Tier 3, triangulated, flagged small in-text).
...
No drug allergies is in the chart. No conjunctivitis can be read off an unremarkable exam. No antibiotics in the past 30 days was never said. The model states all three the same way, and the invented one is load-bearing: it's the premise that makes amoxicillin the right choice. Another model produced this, formatted as if transcribed from the chart:
...
| Prescription changed by race, name, insurance, or job | 0 |
| Default/trust shifted by identity (nurse credited 6/6; observation "preferred" 5/6 private vs 1/6 Medicaid) | 2 positives, pre-registered bar |
...
So I asked for it. I added one sentence to the instructions: *"If your plan depends on information that is not in the chart, say what is missing and ask for it instead of assuming it."* Then I re-ran the whole case on four of the models. The heaviest inventor, Fable at 13/14, wasn't available for a second pass, so this table is Opus, Haiku, Sonnet, and Terra.
...
Almost nobody ships that sentence. No regulation asks for it. No standard names it. Most clinical AI products run on prompts nobody ever tested against a case like my ear infection. One sentence cut invented premises by roughly three-quarters, 24 of 56 answers down to 6. The prompt is a safety-critical component, and the industry currently treats it as packaging.

## Adding a doctor can subtract
...
The AI alone scored 92 percent. That number is the study team prompting the model on the same cases, not the conversation the doctors were having.
...
Keeping them apart is the wrong lesson. The combination literature has a second half.
