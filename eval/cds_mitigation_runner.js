...
const MITIGATION_STEM = `A 24-month-old boy is brought to clinic by his mother for ear pain.
...
async function runVendor(browser, vendorName, matchUrl, newUrl, inputSelector, waitMs) {
  console.log(`\n======================================================`);
  console.log(`STARTING MITIGATION ARM: ${vendorName}`);
  console.log(`======================================================`);
  ...
  await inputEl.fill(MITIGATION_STEM);
  console.log(`Filled mitigation prompt into ${vendorName}.`);
  ...
  const outDir = 'results/cds_eval/mitigation';
  ...
  fs.writeFileSync(`${outFile}.md`, `# ${vendorName} — Prompt Mitigation Arm\n\n${text}`);
...
async function main() {
  ...
  const vendors = [
    ...
    { name: 'Primary AI', match: 'primaryai.world', url: 'https://www.primaryai.world/', selector: 'textarea', wait: 25000 },
  ...
  for (const v of vendors) {
    try {
      ...
      console.error(`Error running mitigation on ${v.name}:`, err);
    }
  }
  
  console.log('\nALL MITIGATION RUNS COMPLETE!');
  process.exit(0);
}
