...
async function main() {
  ...
  try {
    ...
    await page.evaluate(() => {
      ...
      const el = items.find(e => e.innerText.includes('5.6 Luna'));
      if (el) el.click();
    });
    ...
    console.log('Submitted 5_6_luna_medium_rep1. Waiting for completion...');
    ...
    for (let elapsed = 0; elapsed < 120; elapsed += 5) {
      ...
      if (state.isDone && state.len > 150 && elapsed > 8) break;
    }
    ...
    const outPath = 'results/cds/chatgpt_tiers/5_6_luna_medium_rep1';
    ...
    fs.writeFileSync(`${outPath}.md`, `# ChatGPT for Clinicians (5.6 Luna — Medium Thinking — Rep 1)\n\n**Model:** 5.6 Luna\n**Thinking Level:** Medium\n**Replicate:** 1\n**Timestamp:** ${new Date().toISOString()}\n**Case Stem:** Pediatric Acute Otitis Media (24-Month-Old)\n\n---\n\n## Verbatim Output\n\n${text}\n`);
    console.log('SAVED 36th trace successfully!');
