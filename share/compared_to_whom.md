...
Sixteen points above the physicians it was supposed to be assisting. Watch what the doctors did with the tool, because it's painfully recognizable. Many typed one-line queries at it, the way you'd poke a search engine. Some pasted the case and asked it to confirm a conclusion they'd already reached. When the model's answer disagreed with their own read, they mostly went with their own read — which is exactly the override the meta-analysis below predicts, aimed in exactly the wrong direction. The bottleneck wasn't the model. It was the collaboration: two capable parties, no protocol, and the party with the last word was the one having the worse day.
...
Verification turned out to be a habit with tools, not a role you get to assign and forget — and the comfortable phrase "human in the loop" quietly assumes a catcher that neither the kappa nor the Stanford trial can produce on demand.
...
Sketch what the training would actually cover, and notice how little of it is technology. Three habits. First: after any AI plan, ask what it assumed — the four-second question, verbatim if you like. Second: read a flagged assumption as a to-do, not a fact — "assuming no recent amoxicillin, please confirm" means *pick up the phone*, the same way a pending culture means check the culture. Third: when you disagree with the model, disagree with evidence — a finding, a guideline line, a lab value — not with gestalt, because the Stanford data says gestalt-override is where the combined performance bleeds out. The AI side is even shorter: ship the ask-don't-assume sentence, show calibrated uncertainty instead of decoration, and log what the model flagged so someone can audit whether the flags got checked. None of this requires a new model. All of it requires deciding that the collaboration is a clinical skill, taught and tested like any other.
