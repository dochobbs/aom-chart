...
1. **Query Alone Prompts Hallucination in Distilled Models:** Telling the model to query missing information (Clause A) was not only insufficient for flagships (Cell 2: Sonnet 2/3, Opus 1/3), but actively induced confabulation in the distilled model. While Haiku was clean at baseline on head trauma (0/3, defaulting to CT for vomiting), adding Clause A in Cell 2 caused it to invent *"no LOC reported"* in its reassuring findings table (1/3), and adding Clause A+B in Cell 5 caused it to assert a *"witnessed fall"* (1/3). Smaller models appear compelled to answer the queries they formulate.
...
- **Parametric Literature Decay in Compact Architecture:** In Haiku 4.5, **17 of 24 traces (70.8%)** prescribed obsolete pre-2004 45 mg/kg/day amoxicillin, compared to **0 of 72 traces (0.0%)** in frontier models (which universally prescribed 80–90 mg/kg/day).
...
To assess whether the failure mode and curative architecture generalize across commercial developers and model parameter tiers, we deployed the $2^3$ factorial matrix across OpenAI and Google models on both Minor Head Trauma and AOM ($N=144$ traces across `gpt-5.6-terra`, `gpt-5.6-luna`, `gemini-3.1-pro-preview`, and `gemini-3.7-flash`):
