...
async function runVendorVariantRep2(browser, vendor, variant) {
  console.log(`\n------------------------------------------------------`);
  console.log(`RUNNING REP 2: [${vendor.name}] x [${variant.name}] (${variant.id})`);
  console.log(`------------------------------------------------------`);
  ...
  const outFile = path.join(outDir, `${vendorKey}_rep2`);
  ...
  fs.writeFileSync(`${outFile}.md`, `# ${vendor.name} — ${variant.name} (${variant.id}) [Replicate 2]\n\n${text}`);
  console.log(`SUCCESS REP 2 [${vendor.name} - ${variant.id}]: Extracted ${text.length} chars.`);
}
...
async function main() {
  ...
  for (const variant of VARIANTS) {
    console.log(`\n======================================================`);
    console.log(`     STARTING REP 2 VARIANT: ${variant.name} (${variant.id})`);
    console.log(`======================================================`);
    for (const vendor of vendors) {
      try {
        await runVendorVariantRep2(browser, vendor, variant);
        await new Promise(r => setTimeout(r, 4000));
      } catch (err) {
        console.error(`Error running rep 2 for ${vendor.name} on ${variant.id}:`, err);
      }
    }
  }
  
  console.log('\nALL DEMOGRAPHIC REPLICATE 2 RUNS COMPLETED!');
  process.exit(0);
}
